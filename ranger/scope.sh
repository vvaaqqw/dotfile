...
handle_extension() {
    case "${FILE_EXTENSION_LOWER}" in
        ...
        # BitTorrent install transmission
        torrent)
            transmission-show -- "${FILE_PATH}" && exit 5
            exit 1;;

        # OpenDocument yay -S odt2txt
        odt|ods|odp|sxw)
            # Preview as text conversion
...
handle_image() {
    ...
    case "${mimetype}" in
        ...
        image/*)
         ...
         # Video
         video/*)
             # Thumbnail #install ffmpegthumbnailer
        ...
        # ePub, MOBI, FB2 (using Calibre) install calibre
